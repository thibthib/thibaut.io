...
The `<canvas/>` HTML element can be used to draw graphics with a finer control than with the usual DOM or SVG. But trying to draw on a canvas with React is not intuitive, as their interfaces are quite different. With React, we build components that each own their own node. With canvas, there is only one node, and we draw in it. Let's see how we can make a canvas visualisation with React components !

## 🧑‍🏫 Canvas 101

The `canvas` element is like a sheet of paper. To draw in real life, we take a pen, we move our hand to a first position, and draw a line by moving our hand to another position. The browser API to draw on a canvas is actually very similar. We first need to make a blueprint of the shape we want to draw (like using a wood pencil), and then we can color it.
...
Now, both our `Canvas` and `Hexagon` components are ready. We can display several random hexagons on the screen!
...
As we saw, the canvas is like a sheet of paper: once we've drawn on it, we can't change it! But we can clear the canvas and re-draw on it. Animating on a canvas is like animating cartoons. It's a lots of drawings quickly shown ones after the others! To make a shape move, we need to split the movement into small steps. Then, we can draw them one by one, while clearing the canvas between them. Those steps are called frames. Browsers come with an API so that we can draw in each frame: `requestAnimationFrame`.
...
But... the canvas is now white! It's because we only render the hexagons once – when `<RandomHexagons />` is first rendered. Yet, the canvas is cleared at every frame, erasing the hexagons on the second frame. Children components should also draw in the canvas at every frame. To force them to re-render, one solution is to share the `frameCount` from the canvas with the `Hexagon` component. It's achieved via a React context, like we did with `SharingContext`:
...
And now our hexagons are back on the screen! 🎉 While this works, the `FrameContext` has to be added in every child component. Two options are available to make sure those two contexts are being consumed together:
...
We want the hexagons to rotate. It means that each hexagon should have its rotation angle change at every frame, by incrementing it by 1 for example. To do this, we need the hexagons to _remember_ the rotation from the previous render: we will need to use a [React ref](/react-refs-evolution):
...
Like we did with `useCanvas`, we can also improve a bit the readability of this code. We can hide the implementation detail – here, the use of a React ref – in a hook:
...
- At some point it can become quite CPU heavy to try to animate a lot of shapes in a canvas. At this point, there's another more performant way to draw things: WebGL. It's a huge subject on its own! I recommend to look at how we can write WebGL based components with [react-three-fiber](https://github.com/react-spring/react-three-fiber) _— spoiler alert, they made their own React reconciler_

- Our `useAnimation` hook is not helping if we want to animate stuff other than infinitely changing numbers. To create more complex ones, [react-spring](https://github.com/react-spring/react-spring) is the library to go. It needs a bit of wiring to have it work with our own frame loop – [here's how it's done to work with `react-three-fiber` for example](https://github.com/react-spring/react-spring/blob/bdf8afb8b2d837fd756f8ffaae6931fb15c0e3d2/src/targets/three/index.ts#L24-L25).