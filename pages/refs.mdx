...
For React applications, the DOM elements sometime need to be handled directly. One example being focus management: to give an input element focus without a user clicking on it, you need to call [`input.focus()`](https://developer.mozilla.org/en-US/docs/Web/API/HTMLElement/focus). Getting a DOM element in a React app is possible with refs. But new ways to handle refs were released over time, and it became trickier to know the differences between the various APIs. A couple weeks ago, while trying to respond to a PR comment, I realized that some of my assumptions about refs were wrong. So let's stop a minute and examine their evolution !
