...
For React applications, the DOM elements sometime need to be handled directly. One example being focus management: to give an input element focus without a user clicking on it, you need to call [`input.focus()`](https://developer.mozilla.org/en-US/docs/Web/API/HTMLElement/focus). Getting a DOM element in a React app is possible with refs. But new ways to handle refs were released over time, and it became trickier to know the differences between the various APIs. A couple weeks ago, while trying to respond to a PR comment, I realized that some of my assumptions about refs were wrong. So let's stop a minute and analyze their evolution !
...
But this API design was not without some issues. For context, look at the [React issue #1373](https://github.com/facebook/react/issues/1373) or [this comment from @gaearon](https://github.com/facebook/react/pull/8333#issuecomment-271648615) explaining some of the issues, like the inability to get several refs on a single element. So the React team came up with a new API: **Callback refs**.
...
With a much finer control over how to handle refs, most issues from the Strings refs API are now solved! Two components can have their own ref in a single element. This API is, to date, the most flexible way to handle refs.
...
When a component uses `forwardRef`, any parent component can pass to it a `ref` prop in order to get access to an inner element. The parent component being able to use `createRef`, `useRef` or callback refs, the `ref` parameter received from `forwardRef` is of a new combined type:
