...
When a component uses `forwardRef`, any parent component can pass it a `ref` prop and get access to an inner element. The parent component can use `createRef`, `useRef` or callback refs, so the `ref` parameter received from `forwardRef` is of a new type:
...
With the `useImperativeHandle` hook, we can go even further and decide what our component will return via its `ref` prop. It can be pretty useful for some use-cases. For example, some components need a local Ref on an element, as well as forwarding one to its parent. Callback refs can also solve this use-case, but this API simplifies it a lot:
